...
Confluence 화이트보드와 같은 시각적 이벤트스토밍 다이어그램을 Mermaid 플로우차트 코드로 변환하는 과정입니다. 이 과정은 Gemini Vision 모델을 활용하여 이미지 내용을 분석하고, 지정된 스타일 가이드에 따라 Mermaid 코드를 생성합니다. 또한, NotebookLM과 같은 비주얼 처리 모델을 사용해도 되며, 웹 기반 변환 도구를 활용해 Mermaid 코드로 변환한 뒤 가져오는 방식도 허용됩니다.
...
* 선택 사항: NotebookLM 등 비주얼 처리 모델 또는 웹 기반 이미지→Mermaid 변환 도구.
...
2. **Mermaid 변환 요청 (Gemini CLI Agent에게)**:
   ...
   * 생성된 Mermaid 코드는 `product_plan_gen/input/generated_mermaid.md` 파일로 저장됩니다.
   ...
   위 명령어를 실행하여 프롬프트 내용을 저에게 전달합니다. 이후 이미지와 함께 Vision Model에 전달하여 Mermaid 코드를 생성합니다.
...
3. **NotebookLM 또는 웹 기반 도구 사용 시**:

   * PNG→Mermaid 변환을 Gemini Vision 모델 외에도 NotebookLM 등의 비주얼 처리 모델로 수행할 수 있습니다.
   * 웹 기반 변환 서비스를 사용해 Mermaid 코드를 생성한 뒤, 해당 코드를 `generated_mermaid.md` 파일에 수동 저장해도 됩니다.
...
3. **Human-in-the-loop 검수**:
   ...
   * 생성된 파일을 검토한 후, 터미널에 Enter를 눌러 다음 단계로 진행하거나, `exit`를 입력하여 프로그램을 중단할 수 있습니다.
...
* `GEMINI_API_KEY` 환경 변수가 설정되어 있어야 합니다(export로 설정).
